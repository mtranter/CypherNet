namespace CypherNet.UnitTests
{
    ...
    public class CypherQueryTests
    {
        ...
        public void BuildCypherQuery_OptionalMatch_ExecutesCorrectQuery()
        {
            ...
            VerifyCypher(cypher, results.FirstOrDefault(),
                         "START actor=node(1) MATCH (actor)-[:STARED_IN*0..]->(movie) OPTIONAL MATCH (movie)<-[:DIRECTED_BY]-(director) RETURN movie as movie, id(movie) as movie__Id, labels(movie) as movie__Labels");
        }
